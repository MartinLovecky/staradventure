...
class Pagnition
{
    ...
    public function previous_page(): string
    {
        ...
        return "<li class='page-item'><a class='page-link' href='/{$this->selector->action}/{$this->article}/{$previous}#story'>Previous</a></li>";
    }
    ...
    public function main_pagnation(): void
    {
        ...
        for ($x = ($this->currentPage - $range); $x < (($this->currentPage + $range) + 1); $x++) {
            if ($x > 0 && $x <= $totalpages) {
                ...
                echo "<li class='page-item {$active}'><a class='page-link' href='/{$this->selector->action}/{$this->article}/{$x}#story'>{$x}</a></li>";
            }
        }
    }
    ...
    public function next_page(): string
    {
        ...
        return "<li class='page-item'><a class='page-link' href='/{$this->selector->action}/{$this->article}/{$next}#story'>Next</a></li>";
    }
}
